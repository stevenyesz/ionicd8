...
.controller('LoginCtrl', function($scope, $state, $templateCache, $q, $rootScope) {
	$scope.doLogIn = function(){
		$state.go('app.developers',{tid: 1});
	};

	$scope.user = {};

	$scope.user.email = "xfye@72haoyuan.com";
		...
		cordova.plugins.email.isAvailable(
			function (isAvailable) {
				...
				cordova.plugins.email.open({
					to:      'xfye@72haoyuan.com',
					cc:      'lightning_heat@163.com',
					// bcc:     ['john@doe.com', 'jane@doe.com'],
					subject: '苏州开发者ionic+drupal8+restful',
					body:    '用ionic+restful+drupal进行移动应用开速开发'
				});
			}
		);
	};
...
.controller('DevelopersCtrl', function($scope, $stateParams, Article) {
  $scope.developers = Article.query({tid: $stateParams.tid});
})

.controller('DeveloperCtrl', function($scope, $stateParams, Article, Category) {
  // We have to fetch both the article and the image.
  $scope.developer = Article.get({nid: $stateParams.nid}, function(article) {  
  });
